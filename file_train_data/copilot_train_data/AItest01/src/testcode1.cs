...
class Program
{
    ...
    static void ArvoJaTallennaTiedostoon()
    {
        ...
        using (StreamWriter sw = new StreamWriter("datat.txt"))
        {
            for (int i = 0; i < 40; i++)
            {
                double arvo = 1.4 + rand.NextDouble() * (5.8 - 1.4);
                sw.WriteLine(arvo);
            }
        }
    }
    ...
    {
        return File.ReadAllLines("datat.txt").Select(double.Parse).ToArray();
    }

    static void TulostaTiedot(double[] taulukko)
    {
        double summa = taulukko.Sum();
        double keskiarvo = taulukko.Average();
        double minimi = taulukko.Min();
        double maksimi = taulukko.Max();
